...
<template>
  <div class="flex flex-col gap-4">
    <Codemirror
      v-model="script"
      :style="{ height: '750px', background: 'var(--color-purple-950)' }"
      :autofocus="true"
      :indent-with-tab="true"
      :tab-size="2"
    />
    <button class="p-2 bg-purple-700 hover:cursor-pointer" @click="publish('new-script', script)">
      Publish
    </button>
  </div>
</template>
